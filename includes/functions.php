...
function fetch_menu($id, $db) {
	...
	if ($row['parser'] == 'unica') {
		...
		$return = $matches;
		handle_unica_menu($return, $db, $row['id']);
	}
...
//			'&nbsp;\s+(\d{1,2}\.\d{1,2}\.?\s*-\s*\d{1,2}\.\d{1,2}\.?)',
		$regexp_parts = array(
//			'(<span class="breadtext"><p>|<br \/>)(Maanantai|Tiistai|Keskiviikko|Torstai|Perjantai|Lauantai|Sunnuntai).*?(<br \/>|<\/p><\/span>)',
//			'(<span class="breadtext"><p>|<br \/>)(.*?)(<br \/>|<\/p><\/span>)',
		...
		$return = $rows;
		handle_fazeramica_menu($return, $date, $db, $id);
	}
	else {
		return FALSE;
	}
...
function handle_unica_menu($array, $db, $id) {
	$currweek = NULL;
	$currday = NULL;
	$date = NULL;
...
function handle_fazeramica_menu($array, $dateparts, $db, $id) {
	$date = NULL;
	$sql = 'INSERT INTO servings
		(description, diet, price, date, restaurant_id)
		VALUES(:descr, :diet, :price, DATE(:date), :resid)';
